...
def transform_usage_duration_months(df: pd.DataFrame, column_name: str, config=None) -> pd.DataFrame:
    ...
    df_usage_duration_months["score_1"] = (
        df_usage_duration_months["usage_duration_group"]
...
def compute_gmv(df: pd.DataFrame, months_window: int) -> pd.DataFrame:
    ...
    group_cod["avg_don"] = np.where(
        ...
        group_cod["thuho_tongdon"] / group_cod["tongdon_cod"].replace(0, np.nan),
    )
    ...
    # ship_rev_ratio
    ship_rev = (group_cod["don_ptc"] * group_cod["tong_tien"]) / group_cod["tong_don"].replace(0, np.nan)
    ...
    ship_rev_non = (group_non_cod["don_ptc"] * group_non_cod["tong_tien"]) / group_non_cod["tong_don"].replace(0, np.nan)
...
def transform_gmv_12th(df: pd.DataFrame, config=None) -> pd.DataFrame:
    ...
    df_result = df_result.rename(columns={"gmv": "gmv_12th"})
...
def transform_gmv_3th(df: pd.DataFrame, config=None) -> pd.DataFrame:
    ...
    df_result = compute_gmv(df, months_window)
    df_result = df_result.rename(columns={"gmv": "gmv_3th"})
...
def main_transform_data(df, config=None, scoring_month=None) -> pd.DataFrame:    
    ...
    for col in ["avg_success_order_count_per_month",
    ...
    recommend_columns = ["sales_region", "usage_duration_months", "total_success_order_value_per_month"]
    ...
    # result_df["model_version"] = config.get("model_version", "v2.0.0")
